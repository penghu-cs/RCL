...
class SGRAF(object):
    ...
    def __init__(self, opt):
        ...
        # Loss and Optimizer
        if 'max-margin' in opt.loss:
            self.criterion = ContrastiveLoss(margin=opt.margin, max_violation=opt.max_violation)
        else:
            self.criterion = CCL(tau=opt.tau, method=opt.loss, q=opt.margin, ratio=opt.ratio)
    ...
    def forward_sim_batch(self, img_embs, cap_embs, cap_lens, batch_size):
        # Forward similarity encoding
        if img_embs.shape[0] > cap_embs.shape[0]:
            inx = 0
            sims = []
            while inx < img_embs.shape[0]:
                sims += [self.sim_enc(img_embs[inx: inx + batch_size], cap_embs, cap_lens)]
                inx += batch_size
            sims = torch.cat(sims, dim=0)
        else:
            inx = 0
            sims = []
            while inx < cap_embs.shape[0]:
                sims += [self.sim_enc(img_embs, cap_embs[inx: inx + batch_size], cap_lens[inx: inx + batch_size])]
                inx += batch_size
    ...
    def train_emb(self, images, captions, lengths, ids=None, *args):
        ...
        loss = self.forward_loss(sims, batch_len=images.shape[0])
